...
module.exports = {
  ...
  module: {
    rules: [{
      ...
      }, //对.css格式的文件依次应用less-loader，css-loader，style-loader
      {
        test: /\.js$/,
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({ //该插件把HTML或者EJS模板（也可以通过loader指定别的模板）生成一个HTML5文件，通过选项filename设置html生成的路径和名称，并且会自动引入打包生成的js和css文件，当然也可以指定具体引入哪几个。多个模板需要new多个html-webpack-plugin对象
      filename: './index.html', // 生成的html存放路径和名称，相对于出口path
      template: path.resolve(__dirname, './src/view/index.html'), // html模板路径
      inject: true, // js插入的位置，true/'head'/'body'/false
      hash: true, // 为静态资源生成hash值
      chunks: ['common', 'index'], // 设置哪些生成的js/css等文件可以引入html中，不配置就会引入所有生成的js/css等文件
      minify: { // 压缩HTML文件
    ...
    new HtmlWebpackPlugin({
      filename: './list.html',
      template: path.resolve(__dirname, './src/view/list.html'),
      inject: true,
      hash: true,
      chunks: ['common', 'list'],
      minify: {
